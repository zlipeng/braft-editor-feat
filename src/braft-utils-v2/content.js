...
export const removeBlock = (editorState, block, lastSelection = null) => {
  ...
  let nextContentState; let nextEditorState
  const blockKey = block.getKey()
...
export const setSelectionBlockData = (editorState, blockData, override) => {
  ...
  const newBlockData = override ? blockData : ({ ...getSelectionBlockData(editorState).toJS(), ...blockData})
...
export const toggleSelectionLink = (editorState, href, attributes) => {
  ...
  const entityData = {href, ...(typeof attributes === 'object' ? attributes : {})}
